...
const emptyForm: DeviceForm = { name: '', protocol: 'bacnet', address: '', port: '', area: '', auto_start: false }
...
export function DevicesTab() {
  ...
  const openEdit = (d: Device) => {
    setEditId(d.id)
    setForm({ name: d.name, protocol: d.protocol, address: d.address, port: d.port?.toString() ?? '', area: d.area ?? '', auto_start: d.auto_start })
    setError(null)
    setShowForm(true)
  }
      ...
      {showForm && (
        <div style={overlayStyle}>
          <div style={dialogStyle}>
            ...
            <input style={inputStyle} value={form.address} onChange={e => setForm(f => ({ ...f, address: e.target.value }))} placeholder="192.168.1.100" />
